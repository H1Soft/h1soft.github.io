...
privacy/, terms/         약관 — 사이트 공통 + 제품별(qr-scanner/, seukscan/ 하위)
...
1. `js/config.js` 에 앱별 App Store / Google Play URL 입력 → 버튼 자동 활성화
   (QR Scanner는 `APP_STORE_URL`/`PLAY_STORE_URL`, 슥캔은 `SEUKSCAN_*`).
2. 문의 이메일 확정 시 `js/config.js` + `privacy/`·`terms/` 본문 + `index.html` JSON-LD 내 이메일 교체.
3. 온글 제품 페이지 추가 시 홈 카드(`.tool-card--static`)를 링크 카드로 교체.
